...
export function drawGlyph(
  ...
  const x0 = cx - h;
  const y0 = cy - h;
  const line: Options = { ...base, fill: undefined };
  ...
  switch (kind) {
    ...
    case "vm":
      ...
      add(rc.rectangle(x0 + 12 * u, y0 + 16 * u, s - 24 * u, s - 38 * u, { ...line, strokeLineDash: [4, 4] }));
    ...
    case "proxy":
      add(rc.polygon([[cx, y0 + 4 * u], [x0 + s - 4 * u, cy], [cx, y0 + s - 4 * u], [x0 + 4 * u, cy]], base));
    ...
    case "load-balancer":
      ...
      break;
    case "api-gateway":
      add(rc.path(`M${x0 + 8 * u} ${y0 + s - 4 * u} V${y0 + 26 * u} Q${cx} ${y0} ${x0 + s - 8 * u} ${y0 + 26 * u} V${y0 + s - 4 * u} Z`, base));
    ...
    case "database": {
      ...
      const eh = 16 * u;
      add(rc.path(`M${cx - w / 2} ${top} V${bottom} A${w / 2} ${eh / 2} 0 0 0 ${cx + w / 2} ${bottom} V${top}`, base));
    ...
    case "cache": {
      ...
      add(rc.path(`M${cx - w / 2} ${top} V${bottom} A${w / 2} ${8 * u} 0 0 0 ${cx + w / 2} ${bottom} V${top}`, base));
      add(rc.ellipse(cx, top, w, 16 * u, base));
      add(rc.polygon([[cx + 2 * u, y0 + 2 * u], [cx - 10 * u, cy + 4 * u], [cx, cy + 4 * u], [cx - 4 * u, y0 + s - 2 * u], [cx + 12 * u, cy - 4 * u], [cx + 2 * u, cy - 4 * u]], { ...base, fill: "#ffe066", fillStyle: "solid" }));
      break;
    }
    case "queue": {
      const w = s - 4 * u;
    ...
    case "storage": {
      const top = y0 + 12 * u;
      add(rc.polygon([[x0 + 8 * u, top], [x0 + s - 8 * u, top], [x0 + s - 16 * u, y0 + s - 4 * u], [x0 + 16 * u, y0 + s - 4 * u]], base));
    ...
    case "cdn":
      ...
      add(rc.path(`M${x0 + 10 * u} ${cy - 16 * u} Q${cx} ${cy - 10 * u} ${x0 + s - 10 * u} ${cy - 16 * u}`, line));
      add(rc.path(`M${x0 + 10 * u} ${cy + 16 * u} Q${cx} ${cy + 10 * u} ${x0 + s - 10 * u} ${cy + 16 * u}`, line));
      break;
    case "dns":
    ...
    case "user":
      ...
      add(rc.path(`M${x0 + 10 * u} ${y0 + s - 2 * u} Q${x0 + 10 * u} ${y0 + 38 * u} ${cx} ${y0 + 38 * u} Q${x0 + s - 10 * u} ${y0 + 38 * u} ${x0 + s - 10 * u} ${y0 + s - 2 * u} Z`, base));
      break;
    case "internet":
      add(rc.path(`M${x0 + 16 * u} ${y0 + 54 * u} A${12 * u} ${12 * u} 0 0 1 ${x0 + 14 * u} ${y0 + 32 * u} A${14 * u} ${14 * u} 0 0 1 ${x0 + 34 * u} ${y0 + 18 * u} A${16 * u} ${16 * u} 0 0 1 ${x0 + 60 * u} ${y0 + 30 * u} A${12 * u} ${12 * u} 0 0 1 ${x0 + 58 * u} ${y0 + 54 * u} Z`, base));
      break;
    case "monitoring":
      add(rc.rectangle(x0 + 4 * u, y0 + 6 * u, s - 8 * u, s - 24 * u, base));
      add(rc.linearPath([[x0 + 12 * u, y0 + 40 * u], [x0 + 24 * u, y0 + 28 * u], [x0 + 34 * u, y0 + 36 * u], [x0 + 46 * u, y0 + 16 * u], [x0 + 60 * u, y0 + 24 * u]], { ...line, stroke: "#e03131", strokeWidth: 2 }));
...
function isoCube(rc: RoughSVG, cx: number, cy: number, r: number, o: Options): SVGGElement {
  ...
  const dy = r / 2;
  const top: [number, number][] = [[cx, cy - r], [cx + dx, cy - r + dy], [cx, cy], [cx - dx, cy - r + dy]];
  const left: [number, number][] = [[cx - dx, cy - r + dy], [cx, cy], [cx, cy + r], [cx - dx, cy + dy]];
  const right: [number, number][] = [[cx, cy], [cx + dx, cy - r + dy], [cx + dx, cy + dy], [cx, cy + r]];
...
export function arrowHead(rc: RoughSVG, fx: number, fy: number, tx: number, ty: number, o: Options, size = 12): SVGGElement {
  const g = document.createElementNS(SVG_NS, "g");
  const a = Math.atan2(ty - fy, tx - fx);
  for (const d of [Math.PI * 0.82, -Math.PI * 0.82]) {
    g.appendChild(rc.line(tx, ty, tx + Math.cos(a + d) * size, ty + Math.sin(a + d) * size, { ...o, strokeLineDash: undefined }));
  }
  return g;
}

function arrowLine(rc: RoughSVG, x1: number, y1: number, x2: number, y2: number, o: Options): SVGGElement {
  const g = document.createElementNS(SVG_NS, "g");
